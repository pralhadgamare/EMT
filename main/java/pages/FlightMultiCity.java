...
public class FlightMultiCity  
{
    WebDriver driver;
  
    public FlightMultiCity(WebDriver driver) 
    {
        this.driver = driver;
    }
...
//    private By classTraveller = By.xpath("(//span[contains(@class,'chk_flt')])[6]");
...
//    private By deptcity2 = By.id("FromSector-mul2_show");
    ...
    public void departuredate1(String sourceMonth,String sourceDate) throws InterruptedException 
    {
        ...
        driver.findElement(By.xpath("//span[contains(text(),'"+sourceMonth+"')]//following::a[contains(text(),'"+sourceDate+"')]")).click();  //select from date 
      //  Thread.sleep(1000);
    ...
    public void departuredate2(String departureMonth2,String departureDate2) 
    {
        ...
        driver.findElement(By.xpath("//span[contains(text(),'"+departureMonth2+"')]//following::a[contains(text(),'"+departureDate2+"')]")).click();  //select from date 
        System.out.println(departureMonth2+"  "+departureDate2);
        
    }

    public void travellerEconomy() throws FileNotFoundException, IOException, InterruptedException 
    {
        driver.findElement(passengerClick).click();
        String adult = Utility_Class.getpfdata(1, 7, "Flight");
        int x = Integer.parseInt(adult);
        for(int i = 0; i < x; i++) 
        {
            driver.findElement(adultSelect).click();     //adult count ++
//            driver.findElement(By.xpath("((//input[@class='plus_box1'])[2]")).click(); //adult
        }
        System.out.println("Adults = "+x);    //for printing on console

        String child = Utility_Class.getpfdata(1, 8, "Flight");
        int y = Integer.parseInt(child);
        for(int j = 0; j < y; j++) 
        {
            driver.findElement(childSelect).click();    //children count++
//          driver.findElement(By.xpath("(//input[@class='plus_boxChd'])[1]")).click();     //children
        }
        System.out.println("Children = "+y);    //for printing on console
        Thread.sleep(1000);
           
        String classTraveller = Utility_Class.getpfdata(1, 9, "Flight");
        ...
        Thread.sleep(1000);
          ...
          public void searchFlight() // to click on Search Flight Button
    {
